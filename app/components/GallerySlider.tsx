    ...
    <section className="w-full bg-brand-black py-24 border-t border-brand-white/5 overflow-hidden">
      ...
      ">
        ...
        {works.map((work, index) => (
          ...
          <div 
            key={work.id} 
            // CORRECCIÓN 1 y 2: Usamos clases estándar 'aspect-3/4' y 'aspect-4/5'
          ...
          >
            ...
            {/* CORRECCIÓN 3: Eliminamos el conflicto. Dejamos solo 'transition-all' que cubre todo. */}
            ...
            {/* INFO FLOTANTE */}
            {/* CORRECCIÓN 4: 'bg-linear-to-t' es la sintaxis correcta en Tailwind v4 */}
            <div className="absolute bottom-0 left-0 w-full p-8 bg-linear-to-t from-black via-black/50 to-transparent">
              <div className="flex items-center gap-3 mb-2">
                {/* CORRECCIÓN 5: 'h-px' en lugar de h-[1px] */}
                <div className="w-8 h-px bg-brand-bone"></div>
                {/* CORRECCIÓN 6: 'tracking-ultra' en lugar de valor arbitrario */}
                <span className="text-[10px] tracking-ultra uppercase text-brand-bone/80">
                  {work.type}
                </span>
              </div>
              <h3 className="font-serif text-4xl text-brand-white italic mb-1">
                {work.title}
              </h3>
              <p className="font-sans text-xs text-brand-olive uppercase tracking-widest">
                {work.subtitle}
              </p>
            </div>

            {/* Botón 'Ver' */}
            <div className="absolute top-6 right-6 w-10 h-10 rounded-full border border-brand-bone/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-brand-black/50 backdrop-blur-sm">
        ...
        {/* TARJETA FINAL "VER TODO" (Móvil) */}
        {/* CORRECCIÓN 7: aspect-3/4 */}
